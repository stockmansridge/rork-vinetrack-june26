...
-- V1 remains disabled. V2 is the only privileged persistence boundary.
revoke all on function public.recover_spray_row_assignments_v1(uuid,uuid,uuid,jsonb)
  from public, anon, authenticated, service_role;

create unique index if not exists spray_row_recovery_operations_trip_fingerprint_v2_uidx
  on public.spray_row_recovery_operations(trip_id, assignment_fingerprint)
  where assignment_fingerprint like 'v2:%';

create table if not exists public.spray_row_recovery_trip_guard (
  trip_id uuid primary key references public.trips(id) on delete cascade,
  window_started_at timestamptz not null,
  accepted_count integer not null,
  last_started_at timestamptz not null,
  constraint spray_row_recovery_trip_guard_count_check check (accepted_count between 1 and 5)
);

alter table public.spray_row_recovery_trip_guard enable row level security;
revoke all on public.spray_row_recovery_trip_guard from public, anon, authenticated;
grant select, insert, update on public.spray_row_recovery_trip_guard to service_role;
...
begin
  ...
  -- jsonb has stable key ordering; sorting normalized elements makes array order irrelevant.
  ...
  -- Cheap replay checks occur before any trip lock or concurrency guard.
  ...
  -- Never queue on a trip row. One recovery transaction per trip may proceed.
  ...
  -- Recheck after acquiring the guard because a concurrent winner may have committed.
  select * into logical_operation
  from public.spray_row_recovery_operations
  where trip_id=p_trip_id and assignment_fingerprint=fingerprint;
  if logical_operation.operation_id is not null then
  ...
  for a in select value from jsonb_array_elements(canonical_assignments) loop
    ...
    -- A recovered path is immutable historical evidence. Never validate it against
    -- current geometry and never rewrite it.
  ...
  return jsonb_build_object(
    'status',case
      when jsonb_array_length(conflicts)>0 then 'historical_evidence_preserved'
