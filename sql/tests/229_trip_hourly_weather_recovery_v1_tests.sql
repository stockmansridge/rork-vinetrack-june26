...
begin
  ...
  if body not like '%generate_series%' or body not like '%source_kind in (''observed'',''manual'')%' or body not like '%Trip is not a spray trip%' or body not like '%service_role%' then raise exception 'T9: restart-safe spray-only slot recovery missing'; end if;
