...
begin;
do $test$
declare body text; canonical_report regprocedure;
begin
  ...
  -- SQL 229 wraps SQL 228 and renames this canonical implementation. Accept either
  -- valid point in the documented 228 -> test 228 -> 229 -> test 229 sequence.
  canonical_report:=coalesce(
    to_regprocedure('public.get_spray_report_v1_pre_weather_provenance_v1(uuid)'),
    to_regprocedure('public.get_spray_report_v1(uuid)')
  );
  body:=pg_get_functiondef(canonical_report);
  foreach body in array array['%plannedChemicalTotals%','%tankSessions%','%programStep%','%treatedAreaHa%','%fuelConsumptionSource%','%metadataAmendments%'] loop
    if pg_get_functiondef(canonical_report) not like body then raise exception 'T9: canonical field missing: %',body; end if;
  end loop;
