-- 113: Pruning Tracker — optional link from a pruning entry to one Work Task.
--
-- "Record the pruning once, and optionally turn that same submission into a
-- completed Work Task without entering the same details again."
--
--   * pruning_entries.work_task_id — nullable uuid, one Work Task per entry.
--     LOGICAL reference (no hard FK), following the pruning tables'
--     paddock_id convention: the Work Task is created client-side with a
--     client-generated UUID and pushed through its own offline queue, so the
--     pruning-entry RPC may replay BEFORE the work_tasks row exists remotely.
--     A hard FK would make that replay fail and strand the queue. Stable
--     client UUIDs + the idempotent RPCs keep the link consistent.
--   * record_pruning_entry gains p_work_task_id (default null). The link is
--     written on first insert and back-filled idempotently on replay — a
--     replay can never overwrite an existing different link, so retries and
--     offline replays create at most ONE linked task per entry.
--   * set_pruning_entry_work_task — the explicit link / unlink / replace
--     action (clients cannot write pruning_entries directly).
--   * delete_pruning_entry is unchanged: reversing an entry keeps the Work
--     Task; the client asks the user what to do with it (keep / delete via
--     the existing work-task flows and permissions).
--
-- NOTE: the old 15-parameter record_pruning_entry signature is dropped first —
-- keeping both would make PostgREST RPC resolution ambiguous. p_work_task_id
-- has a default, so clients that omit it (current portal) keep working.
...
comment on column public.pruning_entries.work_task_id is
  'Optional link: the Work Task created from this pruning recording (public.work_tasks.id). NULL = no linked task. Logical reference (no FK) so offline replay order between the pruning queue and the work-task queue never matters. One pruning entry creates at most one linked Work Task.';

create index if not exists pruning_entries_work_task_idx
  on public.pruning_entries (work_task_id);

-- ---------------------------------------------------------------------------
-- 2. RPC: record_pruning_entry (adds p_work_task_id, default null)
-- ---------------------------------------------------------------------------
drop function if exists public.record_pruning_entry(uuid, uuid, uuid, uuid, integer, date, text, numeric, timestamptz, timestamptz, text, text, integer, timestamptz, jsonb);
...
-- ---------------------------------------------------------------------------
-- 3. RPC: set_pruning_entry_work_task — explicit link / unlink / replace.
--    (Clients cannot write pruning_entries directly; this is the ONLY way to
--    change an existing entry's task link — e.g. retry after a failed task
--    creation, or an explicit unlink.)
-- ---------------------------------------------------------------------------
