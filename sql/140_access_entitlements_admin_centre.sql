-- =====================================================================
-- 140_access_entitlements_admin_centre.sql
-- =====================================================================
-- Phase 2C (1 of 2) — server-side READ surface for the System Admin
-- "Access & Entitlements" portal page (/admin/access-entitlements).
--
-- The page must NOT fetch raw billing events into the browser and must
-- NOT re-implement the entitlement rules client-side. These RPCs give it
-- paginated, server-filtered, admin-gated views over the objects built
-- in SQL 132–139. The shared resolver get_my_vinetrack_access() is NOT
-- touched — the admin helper below mirrors its validity rules read-only
-- for arbitrary users (the resolver itself is auth.uid()-bound).
--
-- What this migration adds (all ADDITIVE):
--   A. public._admin_effective_access(p_user_id)
--        Internal helper: computes the effective entitlement for ANY
--        user with the exact SQL 135 validity rules (server time only,
--        sandbox excluded, grace honoured, revocation honoured). Never
--        writes audit state. NO client execute grant.
--   B. public.admin_access_users(...)
--        Paginated + server-filtered user directory: identity, auth
--        status, vineyard memberships, effective access, billing source,
--        purchase platform, product, review status, licences.
--   C. public.admin_user_access_detail(p_user_id)
--        One-call JSON detail for the user drawer: identity, vineyard
--        memberships (active + soft-deleted history + pending invites),
--        effective access, every billing source (active and historical,
--        external ids REDACTED first4…last4), licences held, open alerts.
--   D. public.admin_user_access_history(p_user_id, p_limit)
--        Unified timeline: entitlement audit events + billing events +
--        alert lifecycle. Token/receipt-looking keys stripped.
--
-- SECURITY:
--   * Every RPC raises 42501 for non-System-Admins (is_system_admin()).
--   * SECURITY DEFINER, search_path pinned to public.
--   * External subscription/transaction identifiers redacted.
--   * No raw webhook payloads, receipts, tokens or auth secrets.
--   * The helper (_admin_effective_access) has NO client grant.
--
-- ROLLBACK:
--   drop function if exists public.admin_user_access_history(uuid, integer);
--   drop function if exists public.admin_user_access_detail(uuid);
--   drop function if exists public.admin_access_users(text, boolean, text, uuid, text, text, text, integer, integer);
--   drop function if exists public._admin_effective_access(uuid);
-- =====================================================================
...
begin
  if not exists (
    ...
    raise exception 'SQL 140 precondition failed: apply sql/134 first';
  end if;
  if to_regclass('public.billing_provider_events') is null then
    raise exception 'SQL 140 precondition failed: apply sql/133 first';
  end if;
  if to_regclass('public.billing_admin_alerts') is null then
    raise exception 'SQL 140 precondition failed: apply sql/139 first';
  end if;
  if to_regclass('public.vinetrack_entitlement_state') is null then
    raise exception 'SQL 140 precondition failed: apply sql/132 first';
  end if;
end$$;

-- ---------------------------------------------------------------------------
-- A. _admin_effective_access — read-only resolver mirror for ANY user.
--    Validity rules are a 1:1 copy of get_my_vinetrack_access() (SQL 135):
--    sandbox never grants, server-time expiry, provider grace, future-start
--    denial, revoked/expired manual grants excluded, same precedence and
--    deterministic tiebreakers. Unlike the resolver it never writes
--    entitlement state/audit rows (pure read).
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Denial classification from the user's most recent row (any validity) —
  -- same rules as the resolver.
  select case
...
-- No client grant: only the SECURITY DEFINER RPCs below call this helper.

comment on function public._admin_effective_access(uuid) is
  'SQL 140 internal: read-only mirror of the SQL 135 resolver validity rules for an arbitrary user. Never writes state/audit. No client execute grant.';

-- ---------------------------------------------------------------------------
-- B. admin_access_users — paginated, server-filtered user directory.
--    total_count is a window count over the FILTERED set, so the portal can
--    render pagination without a second query. p_limit is clamped to 200 —
--    the browser never receives the whole billing history.
-- ---------------------------------------------------------------------------
...
returns table (
  ...
  vineyards            jsonb,       -- [{vineyard_id,name,role}] (active vineyards only)
  has_access           boolean,
  reason_code          text,
  access_source        text,        -- effective plan tier | 'none'
  plan_code            text,
  plan_name            text,
  billing_provider     text,
  purchase_platform    text,
...
begin
  ...
  enriched as (
    select
      ...
      ea.current_period_end   as e_period_end,
  ...
  filtered as (
    ...
    where (p_has_access is null or en.e_has_access = p_has_access)
      ...
      and (
        ...
        or (p_billing_source = 'trial' and en.e_status = 'trialing')
      ...
      and (
        ...
        or (p_status_filter = 'trial' and en.e_status = 'trialing')
...
comment on function public.admin_access_users(text, boolean, text, uuid, text, text, text, integer, integer) is
  'SQL 140: System-Admin-only paginated user directory for /admin/access-entitlements. Server-side search + filters (access, plan, vineyard, role, billing source, lifecycle/review status). total_count windows the filtered set. Page size clamped to 200. No payloads, receipts or tokens.';

-- ---------------------------------------------------------------------------
-- C. admin_user_access_detail — one-call JSON for the user drawer.
-- ---------------------------------------------------------------------------
...
comment on function public.admin_user_access_detail(uuid) is
  'SQL 140: System-Admin-only one-call JSON detail for the Access & Entitlements user drawer: identity (no tokens/passwords), memberships (active + soft-deleted history + pending invites), effective access, every billing source with REDACTED external ids, licences held, open alerts.';

-- ---------------------------------------------------------------------------
-- D. admin_user_access_history — unified timeline for the drawer.
--    Sources: vinetrack_entitlement_audit (grants/denials/source changes/
--    store lifecycle), vinetrack_billing_events (manual grants, licences,
--    provider events), billing_admin_alerts (created + acknowledged).
--    Token/receipt-looking keys stripped from every payload defensively.
-- ---------------------------------------------------------------------------
...
returns table (
  ...
  source      text,   -- 'entitlement_audit' | 'billing_event' | 'alert'
  event_type  text,
  platform    text,
  detail      jsonb
)
...
comment on function public.admin_user_access_history(uuid, integer) is
  'SQL 140: System-Admin-only unified access/billing timeline for one user: entitlement audit + billing events + alert lifecycle. Receipt/token-looking payload keys stripped; capped at 500 rows.';

commit;

-- =====================================================================
-- VERIFICATION (run after commit, as a System Admin):
--   select * from public.admin_access_users(p_limit => 20);
--   select * from public.admin_access_users(p_search => 'stockman');
--   select * from public.admin_access_users(p_status_filter => 'manual_override');
--   select public.admin_user_access_detail('<user-uuid>');
--   select * from public.admin_user_access_history('<user-uuid>', 50);
-- Non-admin callers: every RPC raises 42501.
-- Client execute on _admin_effective_access: none (revoked).
-- =====================================================================
