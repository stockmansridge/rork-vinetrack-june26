...
struct ActiveTripView: View {
    ...
    private func buildDiagnosticsSnapshot() -> String {
        ...
        lines.append("Local active: \(live.isActive); server active: not loaded in this snapshot")
        lines.append("Device-owned Trip ID: \(store.deviceActiveTripId?.uuidString ?? "none")")
        lines.append("Active tank: \(live.activeTankNumber.map(String.init) ?? "none")")
        lines.append("Filling: \(live.isFillingTank); filling tank: \(live.fillingTankNumber.map(String.init) ?? "none")")
        lines.append("Tank sessions: \(live.tankSessions.map { "\($0.tankNumber):start=\(isoFormatter.string(from: $0.startTime)),end=\($0.endTime.map { isoFormatter.string(from: $0) } ?? "open")" }.joined(separator: "; "))")
        let actualNumbers = SprayTankActualStore.shared.records.filter { $0.tripId == live.id }.map(\.tankNumber).sorted()
        lines.append("Actual tank numbers: \(actualNumbers)")
        let linkedRecord = store.sprayRecords.first { $0.tripId == live.id }
        lines.append("Planned tank numbers: \(linkedRecord?.tanks.map(\.tankNumber).sorted() ?? [])")
        lines.append("Trip block IDs (\(live.paddockIds.count)): \(live.paddockIds.map(\.uuidString).joined(separator: ","))")
        let plannedBlockIds = linkedRecord?.applicationGeometry?.blocks?.blockIds ?? []
        lines.append("Linked Spray Record block IDs (\(plannedBlockIds.count)): \(plannedBlockIds.joined(separator: ","))")
        lines.append("Tank-state reconciliation occurred: \(tracking.lastReconciledTankTripId == live.id)")
        lines.append("Server completion displaced local active state: \(tracking.lastServerCompletedTripId == live.id)")
