...
struct StartTripSheet: View {
    ...
    /// and direction.
    ///
    /// `availablePaths` — derived from the blocks' actual configured row
    /// numbers — is the authoritative universe. The planner walks indices into
    /// that set, so a produced path can never fall outside the selection and a
    /// non-contiguous selection never generates phantom paths through the gap.
    /// The preview and the persisted `trip.rowSequence` both call this.
    private func generatedSequence() -> [Double] {
        guard combinedTotalRows > 0 else { return [] }
        return TripRowSequencePlanner.plannedSequence(
