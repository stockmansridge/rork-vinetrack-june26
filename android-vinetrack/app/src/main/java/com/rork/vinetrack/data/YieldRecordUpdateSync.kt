...
/**
 * Offline replay coordinator for yield-record UPDATE only (Android Stage L-2).
 *
 * The operator edited an existing, already-synced yield record offline (re-authored
 * an estimate or adjusted per-block actuals + notes), or a transient online PATCH
 * failed, and the optimistic edit is already showing locally. This replays the
 * yield PATCH ([YieldRepository.updateYieldRecord]) and nothing else: it never
 * inserts, never deletes, never touches estimation sessions or any other entity.
 * Yield delete remains online-only / parked for L-3.
 *
 * Discriminator: update markers use [PendingEntityType.YIELD_RECORD] / UPDATE so
 * they can never be picked up by the create coordinator (YIELD_RECORD / CREATE)
 * and this coordinator only ever processes YIELD_RECORD / UPDATE rows.
 *
 ...
 * [YieldRecordCreateSync.foldEdit] and never queues an UPDATE — so a create+edit
 ...
 * ([PendingWrite.clientId] = yieldRecordId) and only one unresolved update is
 ...
 * [Payload.clientUpdatedAt]). The full computed `block_results` array and totals
 * travel in the payload so replay preserves the record byte-for-byte. No
 * auth/session/tokens are stored; `created_by` and server-managed audit/sync
 * columns are never carried.
 */
class YieldRecordUpdateSync(
    ...
    /**
     * Full patch payload needed to replay the edit. Carries the editable yield
     * fields plus the stable client id, vineyard scope, block results and the
     * [clientUpdatedAt] the operator saved the edit at (last-writer-wins stamp).
     * `created_by` and server-managed audit/sync columns are deliberately NOT
     * carried.
     */
    @Serializable
    data class Payload(
    ...
    /**
     * Queue (or replace) a yield-record update for later replay. Coalesces by
    ...
    /**
     * Replay every retry-eligible queued yield-record update. No-ops (returns)
    ...
    suspend fun replayAll(onSynced: (HistoricalYieldRecord) -> Unit) {
        ...
        try {
            ...
            for (write in candidates) {
                ...
                if (payload == null) {
                    pending.updateStatus(write.id, PendingWriteStatus.BLOCKED, "Couldn't read the saved yield record edit.")
                    continue
                }
                ...
                if (hasUnresolvedCreate(payload.id)) {
                    pending.updateStatus(
                        ...
                        "Waiting for this yield record to finish saving first.",
                    )
                    continue
                }
                try {
                    val updated = yieldRepo.updateYieldRecord(
                        id = payload.id,
                        season = payload.season,
                        year = payload.year,
                        totalYieldTonnes = payload.totalYieldTonnes,
                    ...
                    retryOrBlock(write, "Sign-in needed to sync this yield record edit.")
                    ...
                    when {
                        e.code in 500..599 -> retryOrBlock(write, "Server error (${e.code}).")
                            ...
                            "The yield record edit was rejected (${e.code}).",
                        )
                    }
                } catch (e: Exception) {
    ...
    /**
     * True when the same yield record still has an unresolved create marker
