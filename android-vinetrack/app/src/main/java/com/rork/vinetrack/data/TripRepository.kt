...
class TripRepository(private val session: SessionStore) {
    ...
    suspend fun createTrip(
        ...
        val body = TripInsert(
            ...
            currentRowNumber = currentRowNumber ?: rowSequence.getOrNull(sequenceIndex),
            nextRowNumber = nextRowNumber ?: rowSequence.getOrNull(sequenceIndex + 1),
    ...
    suspend fun createImportedTrip(
        ...
        val body = ImportedTripInsert(
            ...
            rowSequence = rowSequence,
            sequenceIndex = sequenceIndex,
            currentRowNumber = rowSequence.getOrNull(sequenceIndex),
            nextRowNumber = rowSequence.getOrNull(sequenceIndex + 1),
            totalTanks = totalTanks,
            createdBy = session.userId,
            clientUpdatedAt = now,
        )
